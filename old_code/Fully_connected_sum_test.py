...
def calculate_energy_change(lattice, L, i, j, J_b, h_b, J_s, zealot_spin, magnetization, fully_connected_sum):
    """
    ...
    social_influence = -J_b*fully_connected_sum
...
def calculate_energy_change_zealot(zealot_spin, magnetization, L, J_s, h_s):
    ...
    leader_influence = -J_s*zealot_spin*magnetization  # Should magnetization be normalized somehow here?
    return -2*(leader_influence+leader_field)

def metropolis_step(lattice, L, temp, k_B, J_b, h_b, h_s, J_s, zealot_spin, lookup_table, magnetization, fully_connected_sum):
    """
    Perform one Metropolis step on the lattice and update the magnetization.
    """
    i, j = np.random.randint(0, L), np.random.randint(0, L)
    delta_E = calculate_energy_change(lattice, L, i, j, J_b, h_b, J_s, zealot_spin, magnetization, fully_connected_sum)
    
    if np.random.rand() < np.exp(-delta_E / (k_B * temp)):
        lattice[i, j] *= -1  # Flip the spin
        magnetization += 2*lattice[i, j]
        fully_connected_sum += 2*(magnetization-lattice[i, j])*lattice[i, j]
 
    return magnetization, fully_connected_sum
...
def run_simulation_for_seed(seed, L, N, temp, k_B, J_b, h_b, h_s, J_s, zealot_spin, num_iterations, number_of_MC_steps, lookup_table):
    ...
    fully_connected_sum = 0
    for i in range(len(flattened_lattice)):
        for j in range(i + 1, len(flattened_lattice)):
            fully_connected_sum += flattened_lattice[i] * flattened_lattice[j]
    ...
    for interval in tqdm(range(num_intervals), desc=f"Temp {temp}, Seed {seed}"):
        for step in range(number_of_MC_steps):
            for _ in range(N): 
                magnetization, fully_connected_sum = metropolis_step(lattice, L, temp, k_B, J_b, h_b, h_s, J_s, zealot_spin, lookup_table, magnetization, fully_connected_sum)
    ...
    if remaining_steps > 0:
        ...
        for _ in range(full_zealot_updates):  # Full zealot updates
            for _ in range(N):
                magnetization, fully_connected_sum = metropolis_step(lattice, L, temp, k_B, J_b, h_b, h_s, J_s, zealot_spin, lookup_table, magnetization)
...
h_b= -1
h_s = N
seeds = np.linspace(1,16,16).astype(int).tolist()
number_of_MC_steps = 2
temperatures = np.linspace(0.1,1.5,4).tolist()
burn_in_steps = int((num_iterations/number_of_MC_steps*N)*0.2)
