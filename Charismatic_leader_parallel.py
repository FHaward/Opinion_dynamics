...
def run_simulation_for_seed(seed, L, temp, k_B, J_b, h_b, h_s, J_s, num_iterations, recalculation_interval):
    np.random.seed(seed)
    lattice = np.random.choice([-1, 1], size=(L, L))
    if lattice[(L // 2), (L // 2)] == -1:
        lattice = -lattice
    ...
    for step in tqdm(range(num_iterations), desc=f"Seed {seed}"):
    ...
    return np.array(magnetization_list) / (L * L)

def parallel_run_simulation(L, temp, k_B, J_b, h_b, h_s, J_s, num_iterations, recalculation_interval, seeds):
    ...
    results = Parallel(n_jobs=-1)(  # -1 uses all available cores
        delayed(run_simulation_for_seed)(
            seed, L, temp, k_B, J_b, h_b, h_s, J_s, num_iterations, recalculation_interval
        ) for seed in seeds
    )
    return results


# Post-processing to calculate averages, m_plus, m_minus, g_plus, g_minus
def post_process_results(all_magnetizations):
    final_magnetizations = [magnetization_list[-1] for magnetization_list in all_magnetizations]
    final_magnetizations = np.array(final_magnetizations)

    # Separate magnetizations into m_plus and m_minus
    m_plus = final_magnetizations[final_magnetizations > 0]
    m_minus = final_magnetizations[final_magnetizations < 0]
    ...
    return m_plus_avg, m_minus_avg, g_plus, g_minus, m_plus, m_minus
...
recalculation_interval = (L**2)


# Run simulations in parallel with individual progress bars
all_magnetizations_p = parallel_run_simulation(L, temp, k_B, J_b, h_b, h_s, J_s, num_iterations, recalculation_interval, seeds)

# Post-process the results
m_plus_avg_p, m_minus_avg_p, g_plus_p, g_minus_p, m_plus_p, m_minus_p = post_process_results(all_magnetizations_p)

# Output results
print(f"Average Magnetization of m_plus: {m_plus_avg_p}")
print(f"Average Magnetization of m_minus: {m_minus_avg_p}")
print(f"Fraction g_plus (runs with positive magnetization): {g_plus_p}")
print(f"Fraction g_minus (runs with negative magnetization): {g_minus_p}")
...
plt.plot(average_magnetization_across_runs_p, label="Average Magnetization (across runs)")
